...
class CCompilerOpt(_Config, _Distutils, _Cache, _CCompiler, _Feature, _Parse):
    ...
    def try_dispatch(self, sources, src_dir=None, **kwargs):
        ...
        src_dir : str
            Directory path for the generated headers and wrapped sources.
        ...
        **kwargs : any
        ...
        if self.cc_noopt:
            for src in sources:
                self._generate_config(src_dir, src, [], True)
            return self.dist_compile(sources, [], **kwargs)

        objects = []
        baseline_flags = self.cpu_baseline_flags()
        for src in sources:
            has_baseline, targets, extra_flags = self.parse_targets(src)
            nochange = self._generate_config(src_dir, src, targets, has_baseline)
            for tar in targets:
                tar_src = self._wrap_target(src_dir, src, tar, nochange=nochange)
                flags = extra_flags + self.feature_flags(tar)
                objects += self.dist_compile([tar_src], flags, **kwargs)

            if has_baseline:
                flags = extra_flags + baseline_flags
                objects += self.dist_compile([src], flags, **kwargs)
    ...
    def report(self, full=False):
        ...
        ########## dispatch ##########
        if self.cc_noopt:
            dispatch_rows.append(("Requested", "optimization disabled"))
        else:
    ...
    def _wrap_target(self, output_dir, src, target, nochange=False):
        assert(isinstance(target, (str, tuple)))

        if isinstance(target, str):
            file_name = target_name = target
        else:
            # multi-target
            file_name = '.'.join(target)
            target_name = '__'.join(target)

        wrap_path = "{0}.{2}{1}".format(
            *os.path.splitext(src), file_name.lower()
        )
        if output_dir and not wrap_path.startswith(output_dir):
            wrap_path = os.path.join(output_dir, wrap_path)
        ...
        with open(wrap_path, "w") as fd:
            ...
            """).format(
                ...
                path=os.path.abspath(src), target_defs=target_defs
            ))
        return wrap_path

    def _generate_config(self, output_dir, src, targets, has_baseline=False):
        config_path = os.path.join(src.replace(".c", ".h"))
        if output_dir and not config_path.startswith(output_dir):
