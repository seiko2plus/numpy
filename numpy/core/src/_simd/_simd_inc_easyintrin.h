...
#define SIMD_IMPL_INTRIN_0(NAME, RET)                     \
    ...
    {                                                     \
        if (!simd_arg_parse(args, NULL, 0)) {             \
            return NULL;                                  \
        }                                                 \
        simd_data r = {.RET = npyv_##NAME()};             \
        return simd_data2obj(r, simd_data_##RET);         \
    }
...
#define SIMD_IMPL_INTRIN_1(NAME, RET, IN0)                \
    ...
    {                                                     \
        ...
        if (!simd_arg_parse(args, req_args, 1)) {         \
            return NULL;                                  \
        }                                                 \
        simd_data r = {.RET = npyv_##NAME(                \
            req_args[0].data.IN0                          \
        )};                                               \
        simd_arg_clear(req_args, 1);                      \
        return simd_data2obj(r, simd_data_##RET);         \
    }
...
#define SIMD_IMPL_INTRIN_2(NAME, RET, IN0, IN1)           \
    ...
    {                                                     \
        ...
        if (!simd_arg_parse(args, req_args, 2)) {         \
            return NULL;                                  \
        }                                                 \
        simd_data r = {.RET = npyv_##NAME(                \
            req_args[0].data.IN0,                         \
...
#define SIMD_IMPL_INTRIN_3(NAME, RET, IN0, IN1, IN2)      \
    ...
    {                                                     \
        ...
        if (!simd_arg_parse(args, req_args, 3)) {         \
            return NULL;                                  \
        }                                                 \
        simd_data r = {.RET = npyv_##NAME(                \
            req_args[0].data.IN0,                         \
