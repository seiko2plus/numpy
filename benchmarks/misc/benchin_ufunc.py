...
import os, sys, re, itertools, functools, json, argparse, multiprocessing
import numpy as np
...
class Timing:
    ...
    def __init__(self, ufunc, nsamples, iteration, warmup):
        self._ufunc = utests.ctiming(ufunc, iteration, warmup)
        self._times = utests.ctiming_elapsed(self._ufunc)
        self._nsamples = range(nsamples)
        self._iteration = iteration

    def times(self):
        return np.array(self._times).astype("float64") / self._iteration

    def scale(self, x, by):
        # ctiming_frequency contains the number of clock-cycles per second
        return (x * by) / utests.ctiming_frequency

    def metrics(self, scale=1000):
        times = self.times()
        lx = np.log(times)
        return dict(
            gmean=self.scale(np.exp(lx.sum()/len(times)), scale),
            gstd=self.scale(np.exp(np.std(lx)), scale),
            mean=self.scale(np.mean(times), scale),
            median=self.scale(np.median(times), scale)
        )

    def run(self, *args, **kwargs):
        for _ in self._nsamples:
...
class Benchmark:
    ...
    def run(self, tests):
        for ufunc_name, test_cases in tests.items():
            timing = Timing(
                ufunc=getattr(np, ufunc_name), nsamples=self._nsamples,
                iteration=self._iteration, warmup=self._warmup
            )
            print("Benchmarking ufunc %s, %d cases " % (ufunc_name, len(test_cases)), end='')
            for case in test_cases.values():
                size = case["size"]; strides=case["strides"]; types=case["types"]
                timing.clear()
                try:
                    timing.run(*[
                        self.rand(size * strides[c], t, c)[::strides[c]]
                        for c, t in enumerate(types)
                    ])
                except:
                    pass
                print('.', end='', flush=True)
                case.update(timing.metrics(scale=self._scale))
            print('done')
    ...
    def _compare(self, tests, cmp_tests, append_to):
        for ufunc_name, test_cases in tests.items():
            ...
            for case_name, case in test_cases:
                metric = case[self._metric]
                cmp_metrics, cmp_ratios = [], []
                for cmp_test in cmp_tests:
                    cmp_case = cmp_test.get(ufunc_name, {}).get(case_name, None)
                    if not cmp_case:
                        cmp_metrics.append(None)
                        cmp_ratios.append(None)
                        continue
                    cmp_metric = cmp_case[self._metric]
                    try:
    ...
    def rand(self, size, dtype, prevent_overlap=0):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--nsamples", type=int, default=1000,
    ...
    parser.add_argument("--metric", choices=metric_choices, default="gmean",
                       help="output metric")
    units_choices = dict(s=0, ms=1000, us=1000000)
    ...
    bench = Benchmark(filter=args.filter, strides=args.strides, sizes=args.sizes,
                      nsamples=args.nsamples, iteration=args.iteration, warmup=args.warmup,
                      metric=args.metric, scale=units_choices[args.units])
